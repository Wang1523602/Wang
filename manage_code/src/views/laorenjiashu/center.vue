				...
				<el-row>
					<el-col :span="12">
						<el-form-item label="家属账号" prop="jiashuzhanghao">
							<el-input class="list_inp" v-model="user.jiashuzhanghao" readonly placeholder="家属账号" clearable />
						</el-form-item>
					</el-col>
					<el-col :span="12">
						<el-form-item label="家属姓名" prop="jiashuxingming">
							<el-input class="list_inp" v-model="user.jiashuxingming"  placeholder="家属姓名" clearable />
						</el-form-item>
					</el-col>
					<el-col :span="24">
						<el-form-item label="头像" prop="touxiang">
							<uploads
								...
								@change="laorenjiashutouxiangUploadSuccess">
							</uploads>
						</el-form-item>
					</el-col>
					<el-col :span="12">
						<el-form-item label="性别" prop="xingbie">
							<el-select 
								...
								placeholder="请选择性别"
								>
								<el-option v-for="item in laorenjiashuxingbieLists" :label="item" :value="item"></el-option>
							</el-select>
						</el-form-item>
					</el-col>
					<el-col :span="12">
					...
					<el-col :span="12">
						<el-form-item label="老人账号" prop="laorenzhanghao">
							<el-select 
								...
								placeholder="请选择老人账号"
								>
								<el-option v-for="item in laorenjiashulaorenzhanghaoLists" :label="item" :value="item"></el-option>
							</el-select>
						</el-form-item>
					</el-col>
...
<script setup>
	...
	const tableName = ref('laorenjiashu')
	const user = ref({})
	const laorenjiashuxingbieLists = ref([])
	const laorenjiashulaorenzhanghaoLists = ref([])
	const init = () => {
		laorenjiashuxingbieLists.value = "男,女".split(',')
		context?.$http({
			url:`option/laoren/laorenzhanghao`,
			method:'get'
		}).then(res=>{
			laorenjiashulaorenzhanghaoLists.value = res.data.data
		})
	}
	const laorenjiashutouxiangUploadSuccess=(fileUrls)=> {
	    user.value.touxiang = fileUrls;
	}
	const onSubmit = () => {
		if((!user.value.jiashuzhanghao)){
			context?.$toolUtil.message(`家属账号不能为空`,'error')
			return false
		}
		if((!user.value.mima)){
		...
		if((!user.value.jiashuxingming)){
			context?.$toolUtil.message(`家属姓名不能为空`,'error')
			return false
		}
