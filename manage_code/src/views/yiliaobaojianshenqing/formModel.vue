	...
	<div>
		<el-dialog v-model="formVisible" :title="formTitle" width="50%" destroy-on-close :fullscreen='false'>
			<el-form class="formModel_form" ref="formRef" :model="form" label-width="$template2.back.add.form.base.labelWidth" :rules="rules">
				<el-row>
					<el-col :span="12">
						<el-form-item label="保健名称" prop="baojianmingcheng">
							<el-input class="list_inp" v-model="form.baojianmingcheng" placeholder="保健名称"
								 type="text" 								:readonly="!isAdd||disabledForm.baojianmingcheng?true:false" />
						</el-form-item>
					</el-col>

					<el-col :span="12">
						<el-form-item label="保健类型" prop="baojianleixing">
							<el-select
								class="list_sel"
								:disabled="!isAdd||disabledForm.baojianleixing?true:false"
								v-model="form.baojianleixing" 
								placeholder="请选择保健类型"
								>
								<el-option v-for="(item,index) in baojianleixingLists" :label="item"
									:value="item"
									>
								</el-option>
							</el-select>
					...
					<el-col :span="12">
						<el-form-item label="保健内容" prop="baojianneirong">
							<el-input v-model="form.baojianneirong" placeholder="保健内容" type="textarea"
							:readonly="!isAdd||disabledForm.baojianneirong?true:false"
							/>
						</el-form-item>
					</el-col>
				</el-row>
...
<script setup>
	...
	const disabledForm = ref({
		baojianmingcheng : false,
		baojianneirong : false,
	...
	const init=(formId=null,formType='add',formNames='',row=null,table=null,statusColumnName=null,tips=null,statusColumnValue=null)=>{
		...
		if(formType == 'add'){
			...
			// getInfo()
			for(let x in row){
				if(x=='baojianmingcheng'){
					form.value.baojianmingcheng = row[x];
					disabledForm.value.baojianmingcheng = true;
					continue;
				}
				if(x=='baojianneirong'){
					form.value.baojianneirong = row[x];
					disabledForm.value.baojianneirong = true;
					continue;
				}
				if(x=='baojianleixing'){
					form.value.baojianleixing = row[x];
					disabledForm.value.baojianleixing = true;
					continue;
				}
		...
		context?.$http({
			...
			if(json.hasOwnProperty('jiashuzhanghao')&& context?.$toolUtil.storageGet("role")!="管理员"){
				form.value.jiashuzhanghao = json.jiashuzhanghao
				disabledForm.value.jiashuzhanghao = true;
			}
		})
		context?.$http({
